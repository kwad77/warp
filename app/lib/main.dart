...
///
/// SPEC §17/§18 — the offline check-in and POI-creation outboxes are loaded once at app
/// start, and replayed opportunistically whenever the user is (or becomes) logged in —
/// both `POST /checkins/intent` and `POST /pois` need auth, so there's no point
/// attempting either before that. No background sync: this is the "app launch" trigger
/// both SPECs call for; the other is a manual retry action (ProfileScreen).
...
class _RootScreenState extends ConsumerState<RootScreen> {
  ...
  void initState() {
    ...
    Future.microtask(() => ref.read(poiCreateOutboxControllerProvider.notifier).load());
  }
